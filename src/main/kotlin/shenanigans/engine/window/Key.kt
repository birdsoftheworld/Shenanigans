...
@JvmInline
value class Key internal constructor(val token: Int) {
    companion object {
        val SPACE = Key(32)
...
@JvmInline
value class KeyAction internal constructor(val code: Int) {
    companion object {
        val PRESS = KeyAction(GLFW.GLFW_PRESS)
        val RELEASE = KeyAction(GLFW.GLFW_RELEASE)
        val REPEAT = KeyAction(GLFW.GLFW_REPEAT)
    }
}

@JvmInline
value class KeyModifier internal constructor(val code: Int) {
    companion object {
        val SHIFT = KeyModifier(GLFW.GLFW_MOD_SHIFT)
        val CONTROL = KeyModifier(GLFW.GLFW_MOD_CONTROL)
        val ALT = KeyModifier(GLFW.GLFW_MOD_ALT)
        val SUPER = KeyModifier(GLFW.GLFW_MOD_SUPER)
        val CAPS_LOCK = KeyModifier(GLFW.GLFW_MOD_CAPS_LOCK)
        val NUM_LOCK = KeyModifier(GLFW.GLFW_MOD_NUM_LOCK)
    }
}
