...
class HelloWorld {
    ...
    private fun init() {
        ...
        GLFW.glfwSetKeyCallback(window) { window: Long, key: Int, scancode: Int, action: Int, mods: Int ->
            ...
            if (key == GLFW.GLFW_KEY_D && action == GLFW.GLFW_PRESS) xPos += 10
            if (key == GLFW.GLFW_KEY_A && action == GLFW.GLFW_PRESS) xPos -= 10
            if (key == GLFW.GLFW_KEY_S && action == GLFW.GLFW_PRESS) yPos += 10
            if (key == GLFW.GLFW_KEY_W && action == GLFW.GLFW_PRESS) yPos -= 10
        }
    ...
    private fun loop() {
        ...
        // Set the clear color
        GL11.glClearColor(0.5f, 5.0f, 0.5f, 0.5f)
